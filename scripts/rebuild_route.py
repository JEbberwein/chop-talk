content = open('app/api/chat/route.js', 'r').read()
...
async function getBravesContext() {
  try {
    ...
    const [gameRes, rosterRes, hitRes, starterRes, bullpenRes] = await Promise.all([
      ...
      fetch(base + '/stats?stats=season&group=pitching&season=' + season + '&teamId=' + BRAVES_ID + '&sportId=1&limit=15&startingPitcher=true'),
      fetch(base + '/stats?stats=season&group=pitching&season=' + season + '&teamId=' + BRAVES_ID + '&sportId=1&limit=15&startingPitcher=false'),
    ]);
...
export async function POST(request) {
  try {
    ...
    const rawReply = textBlocks.length > 0 ? textBlocks[textBlocks.length - 1].text : 'Something went wrong.';
...
open('app/api/chat/route.js', 'w').write(new_content)
print('Done')
